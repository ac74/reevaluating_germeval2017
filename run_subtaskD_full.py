...
#from AURC.src.utils import InputFeatures, get_data_with_labels
...
from modeling_CRF import TokenBERT, TokenDistilBERT

logging.basicConfig(filename="subtaskD_full.log", level=logging.INFO)
...
def main(train_df, dev_df, test_syn_df = None, test_dia_df = None, lang_model = 'bert-base-german-dbmdz-uncased', use_crf=False):
    ...
    #parser.add_argument('--train_data', type=str, default='train_df.tsv', help='The input train data.')
    #parser.add_argument('--dev_data', type=str, default='dev_df.tsv', help='The input development data.')
    #parser.add_argument('--test_data1', type=str, default='test_syn.tsv', help='The first input test data (synchronic).')
    #parser.add_argument('--test_data2', type=str, default='test_dia.tsv', help='The sencond input test data (diachronic).')
    ...
    #parser.add_argument('--tag_values', type=str, default='tag_values.json', help='The input tag values.')
    ...
    parser.add_argument('--pretrained_weights', type=str, default='bert-base-german-dbmdz-uncased', help='The pretrained bert model.')
    ...
    seed = 42
    epochs = 1
    batch_size = 8 # 8 for bert-base-multilingual-cased and -uncased
    max_len = 512
    lr = 5e-5
    #use_crf = True
    train_mode = True
    ...
    if train_mode:
        ...
        if model_class=="BERT":
            ...
            config.hidden_dropout_prob = 0.1 # was macht das?            
            # Model
            model = TokenBERT(
        ...
        for epoch in range(epochs):
            ...
            print("TRAIN: F1 %.3f | F1 Overlap %.3f"%(f1s_train, f1s_overlap_train))
            ...
            print("EVAL: F1 %.3f | F1 Overlap %.3f | BEST F1: %.3f"%(f1s_dev, f1s_overlap_dev, best_f1s_dev), best_epoch)
            ...
            print("TEST SYN: F1 %.3f | F1 Overlap %.3f"%(f1s_test_syn, f1s_overlap_test_syn))
            ...
            print("TEST DIA: F1 %.3f | F1 Overlap %.3f"%(f1s_test_dia, f1s_overlap_test_dia))
    ...
    if eval_mode:
        ...
        y_true_dev, y_pred_dev, f1s_dev, f1s_overlap_dev = evaluation(
                dev_dataloader, model=model, device=device, tokenizer=tokenizer, tag_values=tag_values)
        print("EVAL: F1 %.3f | F1 Overlap %.3f"%(f1s_dev, f1s_overlap_dev))
